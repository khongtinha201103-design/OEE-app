...
def load_data():
    ...
    conn = pyodbc.connect(
        ...
        f"PWD={st.secrets.get('DB_PASSWORD', 'Siplace%Sa.1.Pwd')};"
        "TDS_Version=8.0;"
    )
...
if len(machines) > 1 and "1080" not in machines:
    default_selection.append(machines[0])

selected_machines = st.multiselect(
    "Select Machines (choose 2)", machines, default=default_selection
)
...
# Ensure full date range filter applies properly
if len(date_range) == 2:
...
df_timeframe = df[
    (pd.to_datetime(df["Start_Time"]).dt.date >= start_filter)
    & (pd.to_datetime(df["End_Time"]).dt.date <= end_filter)
]
...
for machine in selected_machines[:2]:
    filtered_df = df_timeframe[
        ...
        & (
            (df_timeframe["Recipe"] == selected_recipe)
            if selected_recipe
            else True
        )
    ]
    ...
    start_time = st.time_input(
        f"Start time for {machine}",
        value=datetime.now().time(),
        key=f"start_{machine}",
    )
    end_time = st.time_input(
        f"End time for {machine}",
        value=datetime.now().time(),
        key=f"end_{machine}",
    )
...
for machine in selected_machines[:2]:
    ...
    downtime_main = (
        ...
        .groupby("MappedState")["DurationSeconds"]
        .sum()
        .reset_index()
        .sort_values("DurationSeconds", ascending=False)
    )
    downtime_main["FormattedDuration"] = downtime_main["DurationSeconds"].apply(
        format_duration
    )
    ...
    if main_states:
        selected_main_state = st.selectbox(
            f"Filter Technical Pareto ({machine})",
            main_states,
            key=f"tech_{machine}",
        )
        ...
        downtime_tech_filtered = (
            ...
            .groupby("TechnicalState")["DurationSeconds"]
            .sum()
            .reset_index()
            .sort_values("DurationSeconds", ascending=False)
        )
        downtime_tech_filtered["FormattedDuration"] = downtime_tech_filtered[
            "DurationSeconds"
